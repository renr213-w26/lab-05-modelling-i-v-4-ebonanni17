...
ggplot(births14_habitgiven, aes(x=habit, y=weight)) +
  geom_boxplot() + labs(x="Habit", y="Weight")
```
This data suggests that birth weight is higher in babies whose mothers did not smoke during pregnancy compared to mothers who did smoke. 

c.  Now, fit a linear model that predicts `weight` from `habit`. Provide the tidy summary output below.

```{r}
lin_model <- lm(weight~habit, data=births14_habitgiven)
tidy(lin_model)
```
...
$$
\hat{weight}= 7.2698731-0.5926801 \cdot habit_{smoker}
$$
...
```{r}
lin_model2 <- lm(weight~mage, data=births14_habitgiven)
tidy(lin_model2)
```
...
R finds values of the intercept and slope that minimize the sum of squared residuals. R fits the regression line that makes the least amount of prediction errors. The line through the data should be as close to as many points as possible.  
...
The intercept of 6.78 represents the baby's weight when the mother is 0 years old. The intercept in not meaningful because it is outside the observed ranges of what the mothers age would be. No mother is 0 years old. 

d.  Interpret the slope in the context of the data and the research question.

The slope suggests that for every year of the mother's age the baby's weight will increase by 0.015 on average. This relationship is significant. 
...
The response variable is going to be "parsim". 
...
ggplot(parasites, aes(x=divergence_time, y=parsim)) +
  ...
  labs(x="Divergence time (millions of years)", y= "Parasite similarity")
```

d.  Use the visualization to describe the relationship between the two variables.

The data suggests that as divergence time continues parasite similarity decreases. Also, the similarity become more uniform and less scattered. 
